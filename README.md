...
- 配置（`.env`，默认关闭以免无 GPU 环境起不来）：
  `LLM_ENABLED`、`LLM_BASE_URL`、`LLM_API_KEY`、`LLM_MODEL`、`LLM_TIMEOUT_SECONDS`、
  `LLM_MAX_TOKENS`、`LLM_TEMPERATURE`、`RAG_CONTEXT_TOP_K`、`RAG_MAX_CONTEXT_TOKENS`、
  `RAG_MIN_RERANK_SCORE`
- 权重建议目录：`/root/autodl-tmp/models/Qwen3-8B`（可用 ModelScope 下载；
  Hub 无 `Qwen3-7B`，同系列对应档为 8B）
- 启动模型（不纳入 `make infra-up`）：
  - 脚本：`./scripts/serve_qwen3_vllm.sh`（优先本地目录，否则 Hub id）
  - 可选 Compose profile：`infra/docker-compose.llm.yml`（`--profile llm`）
- API：
  - `POST /api/v1/projects/{pid}/ask`（`stream=false` JSON / `stream=true` SSE：
    `retrieval` → `delta` → `final` / `error`）
  - `GET /api/v1/health/llm`（真实连通性，不虚报）
- 前端：知识检索页保留「检索证据」，新增「带来源问答」；流式展示答案与可点击来源卡
