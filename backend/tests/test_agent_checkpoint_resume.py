...
def test_resume_skips_completed_nodes(db: Session, monkeypatch):
    ...
    # Simulate process restart: NEW service instance (same SessionLocal/test db).
    ...
    assert "initialize_run" in skip_names
    assert "match_company_evidence" in skip_names
