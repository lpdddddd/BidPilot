...
`node_started`、tool 起止、`node_completed` / `node_failed` 后均做短 DB commit。第二个 SQLAlchemy Session 可在 run 进行中读到已提交事件。

**完整 Graph + Service 中途可见性**以 `tests/test_agent_persist_attempt_graph.py::test_full_graph_service_midrun_visibility` 为准：经正式 `AgentRunService.start_run` → `build_graph()` → 工具体内 barrier 阻塞，第二个 Session 在 `search_evidence` 仍为 `running` 时可见 `node_started` / `tool_started`。单节点 helper 测试（如 `test_agent_midrun_visibility.py`）仅覆盖工具持久化层，**不能**单独称为完整 Graph 证明。测试使用 barrier / Event，禁止 `sleep`。

### 持久化 attempt 分配（Step 11 收尾）

节点 `attempt` 的事实来源是数据库 `AgentStep`，不是 checkpoint `retry_counts`：

1. `allocate_node_attempt` 对 `AgentRun` 行 `SELECT … FOR UPDATE`，取该 `(run_id, node_name)` 最大 attempt + 1。
2. Graph 内部 retry、API retry、失败后 resume 共用同一分配器。
3. 唯一约束 `uq_agent_steps_agent_run_id_node_name_attempt`。
4. API retry 创建新 `AgentStep` / 物理 `ToolCall`，保留旧 attempt；逻辑 `call_id` 跨 attempt 稳定；幂等键含 attempt。
5. resume 已完成节点不新增 attempt、不重跑。

### Resume / Retry 原子 claim

`claim_run_execution` 在单事务内锁定 run、校验动作与状态、写入 `execution_claim_token` 并 `status=running` 后提交。仅 `claimed` 才可登记 `BackgroundTasks`；进程内 `_running` 仅作优化。结果区分 `claimed` / `already_running` / `invalid_state` / `not_found_or_forbidden`。登记失败时 `release_execution_claim` 恢复可恢复状态。
